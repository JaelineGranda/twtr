...
mongo_client = MongoClient("mongodb+srv://Jaeline:NeuMay2022@projectdatabase.p8qek.mongodb.net/tweets?retryWrites=true&w=majority")
...
def set_env_var():
    ...
    if 'users' not in g:
        users = os.environ.get("USERS", 'Elon Musk,Bill Gates,Jeff Bezos,Jaeline')
        print('users=', users)
        print('g.users=', list(users.split(',')))
        g['users'] = list(users.split(','))
        print('g.users=', g['users'])
    if 'passwords' not in g:
        passwords = os.environ.get("PASSWORDS", 'Tesla,Clippy,Blue Horizon,NeuMay2022')
...
def atlas_connect():
    ...
    # Python
    client = pymongo.MongoClient("mongodb+srv://Jaeline:<password>@projectdatabase.p8qek.mongodb.net/tweets?retryWrites=true&w=majority")
